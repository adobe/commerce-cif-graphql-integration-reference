...
class ProductLoader {
    ...
    constructor(actionParameters) {
        ...
        let loadingFunction = productSkus => {
            ...
            return Promise.resolve(productSkus.map(productSku => {
                console.debug(`--> Fetching product with sku ${productSku}`);
                return this.__getProductBySku(productSku, actionParameters)
                    .catch(error => {
                        console.error(`Failed loading product ${productSku}, got error ${JSON.stringify(error, null, 0)}`);
                        return null;
                    });
            }));
        };
